...
class Data_base:
    ...
    def generate_reset_token(self, cursor, user_id):
        jti = uuid.uuid4().hex
        cursor.execute(
            "UPDATE users SET token = ?, reset_token = ? WHERE id = ?",
    ...
    def registr(self, cursor, login, email):
        cursor.execute(
            "SELECT login, password, email from users WHERE login = ? OR email = ?",
            (
                login,
                email,
            ),
    ...
    def add_users(self, cursor, login, hash_password, email, data_time):
        cursor.execute(
            "INSERT INTO users (login, password, email, date) VALUES (?, ?, ?, ?)",
